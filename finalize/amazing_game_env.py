...
class AmazingGameEnv(gym.Env):
    def __init__(self):
        ...
        self.cube = {"x": random.randint(0,self.SCREEN_WIDTH), "y": 500, "size": 20, "speed": 0, "gravity": 300, "jump": -100, "xSpeed": 0}
    ...
    def get_state(self) -> np.ndarray:
        return np.array([self.cube["x"] + self.cube["size"] / 2, self.cube["y"] , abs((self.currentGap["x"] + (self.gapWidth / 2)) - (self.cube["x"] + self.cube["size"] / 2)) , abs(self.cube["y"] - self.currentGap["y"])], dtype=np.float32)
    ...
    def step(self, action):
        ...
        if action == 0:  # Move left and jump
            self.cube["xSpeed"] = 0
            self.cube["xSpeed"] -= self.cube["jump"] * 0.8
        ...
        if abs(self.cube["y"] - self.currentGap["y"]) < 20:
            reward += 0.5
        elif abs(self.cube["y"] - self.currentGap["y"]) < 50:
            reward += 0.3
        elif abs(self.cube["y"] - self.currentGap["y"]) < 100:
            reward += 0.1
        else:
            reward -= 0.1
    ...
    def render(self, mode: str = "human"):
        if mode == "human":
            ...
            pygame.draw.circle(self.screen, (255,255,255), (self.currentGap["x"] , self.currentGap["y"] - self.camera["y"]), 5)
