...
class AmazingGameEnv(gym.Env):
    def __init__(self, use_pixels: bool = False):
        super().__init__()
        pygame.init()
        ...
        self.use_pixels = use_pixels  # Use pixel or vector observations
    ...
    def get_state(self) -> np.ndarray:
        if self.use_pixels:
            ...
            return np.array([self.cube["x"], self.cube["y"], self.currentGap["x"] + (self.gapWidth / 2), self.currentGap["y"]], dtype=np.float32)
